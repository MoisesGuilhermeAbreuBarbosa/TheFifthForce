...
- [Data Fig 3a.csv](https://zenodo.org/api/records/10995225/files/Data%20Fig%203a.csv/content): block accelerations and reported uncertainties, used for the main fit.
- [Data Fig 3b.csv](https://zenodo.org/api/records/10995225/files/Data%20Fig%203b.csv/content): histogram, retained but not treated as independent measurements.
- [Data Fig 3d.csv](https://zenodo.org/api/records/10995225/files/Data%20Fig%203d.csv/content): three wavepacket-separation group summaries, analyzed separately, not combined with their parent data.
...
For independent reported uncertainties, define w_i=1/sigma_i^2 and fit a constant mu=sum(w_i y_i)/sum(w_i). Results computed here:
...
| Error after multiplying by sqrt(reduced chi-squared) | 5.610010 nm/s² |
...
For an additional switched acceleration A, use

    chi2(A,b,n) = sum_i [(y_i - 35.2 - A - b - n)/(s sigma_i)]^2
                 + (b/2.66)^2 + (n/1.0)^2,
    s = sqrt(587.021211/552).
...
Here b and n are shared systematic and Newtonian-prediction nuisance offsets. They must not be added independently to every block's variance. Profiling them gives
    ...
    A_hat = -2.103815 nm/s²,
    sigma_A = 6.288705 nm/s²,
    Delta chi2(A) = (A - A_hat)^2/sigma_A^2.
...
For V(r)=-G m1 m2[1+alpha exp(-r/lambda)]/r, the exact static free-space field equation for the Yukawa potential per unit test mass is

    (Laplacian - lambda^-2) Phi_Y = 4 pi G alpha rho.

Its Green-function solution is -G alpha integral rho(x') exp(-|x-x'|/lambda)/|x-x'| d³x'. On the cylinder axis, integrate the source height analytically and the radial coordinate by adaptive quadrature. Define

    d_minus = sqrt(r²+(z-L/2)²), d_plus = sqrt(r²+(z+L/2)²),
    theta(r) = 2 pi - 2 asin(width/(2r)),
    K_raw(lambda,z) = G rho integral_[R1,R2] r theta(r)
        [exp(-d_minus/lambda)/d_minus - exp(-d_plus/lambda)/d_plus] dr.
...
To obtain a conditional response, normalize the geometrical range dependence to the published Newtonian prediction:
    ...
    K(lambda) = 35.2 K_raw(lambda,14 mm)/K_raw(infinity,14 mm),
    A = alpha K(lambda).
...
For the [Nelson-Walsh chameleon-vector model](https://arxiv.org/abs/0802.0762), in natural units, with scalar amplitude f, scalar charge q, coupling g, scalar mass squared m_s² and quartic epsilon,
    ...
    Laplacian f = (m_s² + epsilon f² - q² omega²) f,
    Laplacian omega = -g² n + 2 q² g² omega f²,
    m_V²(x) = 2 q² g² f²(x).
