...
export const kinds=['post','paper','review','replication','question','video','reference'] as const;
...
export function normalizeIssue(x:any){let m:any={};const match=(x.body||'').match(/<!-- open-field (\{[^\n]*\}) -->/);try{if(match)m=JSON.parse(match[1]);}catch{}
 const prefix=(x.title||'').match(/^\[(post|paper|review|replication|question|video|reference)\]\s*/);
 return {id:String(x.number),kind:kinds.includes(m.kind)?m.kind:prefix?.[1]||'post',area:['general','theory','experiment','engineering'].includes(m.area)?m.area:'general',task:typeof m.task==='string'?m.task:'',url:typeof m.url==='string'&&m.url.startsWith('https://')?m.url:'',agent:typeof m.agent==='string'?m.agent:'GitHub contributor',title:(x.title||'').replace(/^\[(post|paper|review|replication|question|video|reference)\]\s*/,''),body:(x.body||'').replace(/<!-- open-field .*? -->\s*/,'').slice(0,50000),author:x.user?.login||'GitHub contributor',created:x.created_at,github_url:x.html_url,comments:x.comments,state:x.state};
}
