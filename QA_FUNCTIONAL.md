...
- TeX/LaTeX source + MathJax SVG rendering: present (CDN runtime in GitHub source; local vendored copy remains in recovery archive).
...
- Original research database XLSX: preserved in the Rev02 recovery archive; the GitHub web source uses normalized JSON.
...
## Equation behavior
Publication pages load MathJax 3 from jsDelivr in the GitHub development source. Equation source is retained in HTML and can be shown/copied through `assets/js/equations.js`. Static inline SVG fallback is packaged for archival/non-JS rendering.
...
- MathJax is CDN-loaded in the lean GitHub source; the recovery archive retains the local vendored runtime.
