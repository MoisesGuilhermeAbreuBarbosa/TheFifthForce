...
def cylinder_analytic(z):
    return 2*np.pi*G*rho*(np.hypot(R2,z-L/2)-np.hypot(R1,z-L/2)-np.hypot(R2,z+L/2)+np.hypot(R1,z+L/2))*1e9
z0=.014
...
for lam in [.001,.003,.005,.01,.03,.1,1.,np.inf]:
    ...
    scan.append({'range_m':float(lam) if np.isfinite(lam) else 'infinity','response_nm_s2_per_alpha':response,'alpha_hat':excess/response,'alpha_sigma':total_sigma/response,'alpha_95_lower':interval[0]/response,'alpha_95_upper':interval[1]/response,'abs_alpha_folded95':folded95/response,'relative_response_z13mm':(acceleration(.013,lam)/acceleration(.013))/(response/baseline),'relative_response_z15mm':(acceleration(.015,lam)/acceleration(.015))/(response/baseline)})
spectra=[]
for N in [16,32,64,128]:
    ...
    spectra.append({'N_per_axis':N,'lowest_regulator_frequency_Hz':freq,'exact_regulator_frequency_Hz':exact,'relative_error':(freq/exact-1)})
results={'data_md5':hashes,'n':len(y),'mean_nm_s2':mean,'formal_se_nm_s2':se,'scaled_se_nm_s2':se_scaled,'chi2':chisq,'dof':dof,'reduced_chi2':chisq/dof,'goodness_of_fit_p':chi2.sf(chisq,dof),'excess_nm_s2':excess,'excess_sigma_nm_s2':total_sigma,'excess_95_interval_nm_s2':interval,'folded_absolute_excess95_nm_s2':folded95,'null_delta_chi2':(excess/total_sigma)**2,'leave_one_out_mean_range':[loo.min(),loo.max()],'separation_slope_nm_s2_per_um':coef[1],'separation_slope_sigma':np.sqrt(cov[1,1]),'separation_linear_chi2_1dof':group_chi,'nominal_geometry_newton_nm_s2':a_nominal,'geometry_quadrature_change_nm_s2':convergence,'geometry_normalization_to_published':baseline/a_nominal,'conditional_yukawa_scan':scan,'proca_regulator_spectrum':spectra}
