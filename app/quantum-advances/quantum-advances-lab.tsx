...
type ServerRun={runId:string;contentHash:string;createdAt:string;engine:string;summary:Record<string,number>;points:SimPoint[]};
type ImportedDataset={name:string;rows:number;columns:string[];meanResidual:number;rmsResidual:number;contentHash:string};
...
export function QuantumAdvancesLab(){
  ...
  async function runOnServer(){setServerBusy(true);try{const response=await fetch('/api/quantum-advances/run',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({...params,mode:'simulation'})});const result=await response.json();if(!response.ok)throw new Error(result.error);setServerRun(result);setData(result.points);await append('server_simulation_run',{runId:result.runId,contentHash:result.contentHash,engine:result.engine,input:params,summary:result.summary});setLedgerStatus(`Permanent run ID assigned · ${result.runId}`);}catch(error){setLedgerStatus(`Server run failed · ${String(error)}`);}finally{setServerBusy(false);}}
  ...
  async function importDataset(file:File){const text=await file.text(),lines=text.trim().split(/\r?\n/);if(lines.length<2){setLedgerStatus('Dataset needs a header and at least one data row.');return;}const columns=lines[0].split(',').map(x=>x.trim());const residualIndex=columns.findIndex(x=>['residual','acceleration','signal'].includes(x.toLowerCase()));if(residualIndex<0){setLedgerStatus('CSV needs a residual, acceleration, or signal column.');return;}const values=lines.slice(1).map(line=>Number(line.split(',')[residualIndex])).filter(Number.isFinite);if(!values.length){setLedgerStatus('No numeric measurements found in the selected signal column.');return;}const contentHash=await digest(text),summary={name:file.name,rows:values.length,columns,meanResidual:mean(values),rmsResidual:rms(values),contentHash};setDataset(summary);await append('dataset_imported',summary);setLedgerStatus(`Dataset fingerprinted · ${contentHash.slice(0,20)}…`);}
  function download(name:string,value:unknown){const blob=new Blob([typeof value==='string'?value:JSON.stringify(value,null,2)],{type:'application/json'}),url=URL.createObjectURL(blob),a=document.createElement('a');a.href=url;a.download=name;a.click();URL.revokeObjectURL(url);}
  function exportQpuJob(){const manifest={schemaVersion:'qagra.qpu-job.v1',jobId:`qpu_${crypto.randomUUID()}`,createdAt:new Date().toISOString(),provider,executionStatus:'not_submitted',shots:4096,qubits:4,inputAngles:qInput,openqasm3:qasm(qInput),scientificGuardrail:'This manifest is an execution request, not evidence of physical-QPU execution or anomalous gravity.'};download(`qagra-${provider}-job.json`,manifest);void append('qpu_job_manifest_exported',{jobId:manifest.jobId,provider,executionStatus:manifest.executionStatus,shots:manifest.shots});}
  function publishUrl(){const latest=ledgerRef.current.at(-1);if(!latest)return '#';const title=`[research-receipt] QAGRA run ${String(latest.payload.runId||latest.id)}`;const body=`<!-- open-field ${JSON.stringify({kind:'post',area:'experiment',task:'QAGRA Quantum Advances',agent:'QAGRA web laboratory'})} -->\n\n## QAGRA research receipt\n\n\`\`\`json\n${JSON.stringify({schemaVersion:'qagra.receipt.v1',entry:latest,chainLength:ledgerRef.current.length},null,2)}\n\`\`\`\n\nSubmitted as unreviewed research. Simulation or computation is not physical evidence.`;return `https://github.com/MoisesGuilhermeAbreuBarbosa/TheFifthForce/issues/new?${new URLSearchParams({title,body})}`;}
  ...
  return <div className={styles.lab}>
    ...
    <section className={styles.section}>
      ...
      <div className={styles.actions}><button className="primary" onClick={run}>Run locally</button><button className="primary" onClick={runOnServer} disabled={serverBusy}>{serverBusy?'Running on server…':'Run reproducibly on server'}</button><button className="outline" onClick={runSweep}>Record sweep</button></div>
      {serverRun&&<div className={styles.runReceipt}><div><span>Permanent run ID</span><strong>{serverRun.runId}</strong></div><div><span>Content fingerprint</span><code>{serverRun.contentHash.slice(0,28)}…</code></div><div><span>Execution engine</span><strong>{serverRun.engine}</strong></div></div>}
    ...
    <section className={styles.section}>
      <div className={styles.sectionHead}><div><p className="eyebrow">03 · DATASET INGESTION</p><h2>Fingerprint real or synthetic measurements</h2></div><span className={styles.status}>{dataset?`${dataset.rows} rows`:'CSV · private until published'}</span></div>
      <p>Import a CSV containing a <code>residual</code>, <code>acceleration</code>, or <code>signal</code> column. Processing remains in this browser; the ledger records only its filename, summary, and SHA-256 fingerprint—not the raw measurements.</p>
      <label className={styles.upload}>Select measurement dataset<input type="file" accept=".csv,text/csv" onChange={event=>{const file=event.target.files?.[0];if(file)void importDataset(file);}}/></label>
      {dataset&&<div className={styles.metrics}><div><span>Rows analyzed</span><strong>{dataset.rows}</strong></div><div><span>Mean</span><strong>{fmt(dataset.meanResidual)}</strong></div><div><span>RMS</span><strong>{fmt(dataset.rmsResidual)}</strong></div><div><span>Fingerprint</span><strong>{dataset.contentHash.slice(0,12)}…</strong></div></div>}
    </section>

    <section className={styles.section}>
      <div className={styles.sectionHead}><div><p className="eyebrow">04 · PARAMETER SWEEP</p><h2>Map detectability before touching hardware</h2></div><span className={styles.status}>31 coupling values</span></div>
    ...
    <section className={styles.section}>
      <div className={styles.sectionHead}><div><p className="eyebrow">05 · QUANTUM CIRCUIT</p><h2>Execute the feature map</h2></div><span className={styles.status}>4 qubits · state-vector</span></div>
      ...
      <div className={styles.hardware}><label>Hardware target<select value={provider} onChange={event=>setProvider(event.target.value)}><option value="openqasm">Generic OpenQASM 3</option><option value="ibm-quantum">IBM Quantum</option><option value="amazon-braket">Amazon Braket</option><option value="ionq">IonQ</option><option value="quantinuum">Quantinuum</option></select></label><button className="outline" onClick={exportQpuJob}>Export QPU job manifest</button><p>Export creates a provider-ready, auditable request. It does not submit credentials or claim hardware execution.</p></div>
    </section>

    <section className={styles.section}>
      <div className={styles.sectionHead}><div><p className="eyebrow">06 · BENCHMARK</p><h2>Quantum vs. classical, measured—not assumed</h2></div><span className={styles.status}>Synthetic 4-class task</span></div>
    ...
    <section className={styles.section}>
      <div className={styles.sectionHead}><div><p className="eyebrow">07 · EVIDENCE LEDGER</p><h2>Publish a durable research object</h2></div><span className={styles.status}>{ledger.length} session entries</span></div>
      <p>The working chain is stored privately in this browser and verified with SHA-256. Server runs receive stable content-derived IDs. When ready, publish the latest signed receipt to the project&apos;s GitHub review queue, where it becomes durable, public, discussable, and explicitly unreviewed.</p>
      <div className={styles.actions}><button className="outline" onClick={verifyLedger}>Verify chain</button><button className="outline" onClick={exportLedger} disabled={!ledger.length}>Export JSON</button><a className="primary" href={publishUrl()} target="_blank" rel="noreferrer" aria-disabled={!ledger.length} onClick={event=>{if(!ledger.length)event.preventDefault();}}>Publish latest receipt</a><button className="outline" onClick={clearLedger} disabled={!ledger.length}>Clear local ledger</button></div><p className={styles.ledgerStatus}>{ledgerStatus}</p>
